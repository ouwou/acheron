...
bool OpusDecoder::init(int sampleRate, int channels)
{
    ...
    frameBytes = frameSamples * frameChannels * static_cast<int>(sizeof(float));
    int error;
...
QByteArray OpusDecoder::decodePlc()
{
    ...
    pcm.resize(samples * frameChannels * static_cast<int>(sizeof(float)));
    return pcm;
}
