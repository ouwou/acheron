...
QByteArray OpusEncoder::encode(const QByteArray &pcmFrame)
{
    if (!encoder)
        return {};
    int expectedSize = frameSamples * frameChannels * static_cast<int>(sizeof(float));
    ...
    int bytes = opus_encode_float(encoder,
                                  reinterpret_cast<const float *>(pcmFrame.constData()),
                                  frameSamples,
